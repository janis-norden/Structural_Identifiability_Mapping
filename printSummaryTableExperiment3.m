function printSummaryTableExperiment3(filenames_results, options)
    ...
    for i=1:length(fn)
        ...
        numTrainMin = results.resultsSVM.experimentPars.numTrExVec(1);

        % dense grid: find mean generalization error for ID and UID systems
        meansErrorUIDDense = mean(results.resultsSVM.outcomes.SVM.gen_error(:, :, 1), 2);
        stdErrorUIDDense = std(results.resultsSVM.outcomes.SVM.gen_error(:, :, 1), 0, 2);
        meansErrorUIDSIMDense = mean(results.resultsSVM.outcomes.SVM_SIM.gen_error(:, :, 1), 2);
        stdErrorUIDSIMDense = std(results.resultsSVM.outcomes.SVM_SIM.gen_error(:, :, 1), 0, 2);
        ...
        meansErrorUIDSparse = mean(results.resultsSVM.outcomes.SVM.gen_error(:, :, 2), 2);
        stdErrorUIDSparse = std(results.resultsSVM.outcomes.SVM.gen_error(:, :, 2), 0, 2);
        meansErrorUIDSIMSparse = mean(results.resultsSVM.outcomes.SVM_SIM.gen_error(:, :, 2), 2);
        stdErrorUIDSIMSparse = std(results.resultsSVM.outcomes.SVM_SIM.gen_error(:, :, 2), 0, 2);
        ...
        meansErrorUIDIrr = mean(results.resultsSVM.outcomes.SVM.gen_error(:, :, 3), 2);
        stdErrorUIDIrr = std(results.resultsSVM.outcomes.SVM.gen_error(:, :, 3), 0, 2);
        meansErrorUIDSIMIrr = mean(results.resultsSVM.outcomes.SVM_SIM.gen_error(:, :, 3), 2);
        stdErrorUIDSIMIrr = std(results.resultsSVM.outcomes.SVM_SIM.gen_error(:, :, 3), 0, 2);
