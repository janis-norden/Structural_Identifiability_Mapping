function printSummaryTableExperiment2(filenames_results, options)
    ...
    for i=1:length(fn)
        ...
        SNRVec = results.resultsUID.data.dataGenPars.SNRVec;
        SNRVecLog = log10(SNRVec);

        % find mean generalization error at every level of SNR
        ...
        disp(strrep([fn{i}, ' & ', num2str(numExUsed, '%i'), ' & ', num2str(SNRVecLog(maxIdx), '%.1f'), ' & ' num2str(maxDiff, fspec), ' & ', num2str(meanDiff, fspec), ' \\'], '0.', '0.'))
